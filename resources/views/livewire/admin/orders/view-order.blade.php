            ...
            <div class="w-full flex">
                <div class="flex flex-wrap px-4 items-center justify-between w-full gap-3 container-fluid">
                    ...
                    <div class="flex items-center gap-1">
                        <a href="{{ url('/admin/orders/print/' . $order->id . '/1') }}" target="__blank"
                            class="btn btn-light btn-sm">{{ $lang->data['print_invoice'] ?? 'Print Invoice' }}</a>
                        <a href="{{ url('/admin/orders/print/' . $order->id . '/2') }}" target="__blank"
                            class="btn btn-light btn-sm">{{ $lang->data['print_kot'] ?? 'Print Kot' }}</a>
                        <a href="{{ route('admin.orders') }}"
                            class="btn btn-light btn-sm">{{ $lang->data['back'] ?? 'Back' }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div>
        {{-- <div class="row mb-2 mb-xl-3">
            ...
            <div class="col-auto ms-auto text-end mt-n1">
                <a href="{{ url('/admin/orders/print/' . $order->id . '/1') }}" target="__blank"
                    class="btn btn-success">{{ $lang->data['print_invoice'] ?? 'Print Invoice' }}</a>
                <a href="{{ url('/admin/orders/print/' . $order->id . '/2') }}" target="__blank"
                    class="btn btn-primary">{{ $lang->data['print_kot'] ?? 'Print Kot' }}</a>
                ...
                <div class="col-xl-12">
                    <div class="card mt-3 mx-3">
                        <div class="card-body">
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="view-f4">
                                                                <font class="view-f5">
                                                                    <font class="view-f6">
                                                                        {{ $lang->data['company_info'] ?? 'Company Info' }}
                                                                    </font>
                                                                </font><br>
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                                                            ...
                                                            <td class="view-f7">
                                                                <font class="view-f8">
                                                                    <font class="view-f6 ">
                                                                        {{ $lang->data['invoice_info'] ?? 'Invoice Info' }}
                                                                    </font>
                                                                </font><br>
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                                                                ...
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                                                                        {{ $lang->data['payment_status'] ?? 'Payment Status' }}:
                                                                    </font>
                                                                </font><br>
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                                                            ...
                                                            <td class="view-f9">
                                                                ...
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                                                                        {{ $order->OrderStatusString($order->status) }}
                                                                    </font>
                                                                </font><br>
                                                                <font class="view-align1">
                                                                    <font class="view-f3">
                            ...
                            <div class="row">
                                <table class="table table-auto table-border mt-3">
                                    <thead>
                                        <tr class="text-sm">
                                    ...
                                    <tbody>
                                        @foreach ($order->details as $item)
                                            <tr class="text-sm">
                                                ...
                                                <td>
                                                    ...
                                                    @if ($item->addons->where('type', 2)->first())
                                                        ...
                                                        @foreach ($item->addons->where('type', 2) as $row)
                                                            {{ $row->addon_name }} :
                                                            {{ getCurrency() }}{{ $row->addon_price }} @if (!$loop->last)
                                                                ,
                                                            @endif
                                                        @endforeach
                                                    @endif
                                                ...
                                                <td class="text-end">
                                                    {{ getCurrency() }}{{ number_format($item->rate, 2) }}</td>
                                                <td class="text-center">
                                                    <span class="px-1">{{ $item->quantity }}</span>
                                                </td>
                                                <td class="text-end">
                                                    {{ getCurrency() }}{{ number_format($item->total, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                {{ $lang->data['subtotal'] ?? 'Sub Total' }}:</td>

                                            <td class="text-end">
                                                {{ getCurrency() }}{{ number_format($order->subtotal, 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                {{ $lang->data['discount'] ?? 'Discount' }}:</td>

                                            <td class="text-end">
                                                {{ getCurrency() }}{{ number_format($order->discount, 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                {{ $lang->data['service_charge'] ?? 'Service Charge' }}:</td>

                                            <td class="text-end">
                                                {{ getCurrency() }}{{ number_format($order->service_charge, 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                {{ $lang->data['tax_total'] ?? 'Tax Total' }}
                                                ({{ number_format($order->tax_percentage, 2) }}%):</td>

                                            <td class="text-end">
                                                {{ getCurrency() }}{{ number_format($order->tax_amount, 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                <strong>{{ $lang->data['bill_total'] ?? 'Bill Total' }}:</strong>
                                            </td>

                                            <td class="text-end">
                                                <strong>{{ getCurrency() }}{{ number_format($order->total, 2) }}</strong>
                                            </td>
                                        </tr>
