                ...
                <div class="flex flex-wrap px-4 items-center justify-between w-full gap-3 container-fluid">
                    <div class="flex flex-wrap items-center gap-1 lg:gap-5">
                        <h1 class="text-lg font-medium text-gray-900">
                            {{ $lang->data['customer_report'] ?? 'Customer Report' }}
                        </h1>
                    </div>
                            ...
                            <tbody>
                                @foreach ($data as $customer)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $customer['name'] }}</td>
                                        <td>{{ getCurrency() }}{{ number_format($customer['orders_sum_total'], 2) }}
                                        </td>
                                        <td>{{ getCurrency() }}{{ number_format($customer['payments_sum_amount'], 2) }}
                                        </td>
                                        <td>{{ getCurrency() }}{{ number_format($customer['orders_sum_total'] - $customer['payments_sum_amount'], 2) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
