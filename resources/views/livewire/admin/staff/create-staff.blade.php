                ...
                <div class="flex flex-wrap px-4 items-center justify-between w-full gap-3 container-fluid">
                    <div class="flex flex-wrap items-center gap-1 lg:gap-5">
                        <h1 class="text-lg font-medium text-gray-900">
                            {{ $lang->data['create_new_staff'] ?? 'Create New Staff' }}
                        </h1>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-5 px-4">
                    <div class="flex flex-col gap-2.5">
                        ...
                        <input type="text" class="input"
                            placeholder="{{ $lang->data['enter_staff_name'] ?? 'Enter Staff Name' }}"
                            wire:model="name" />
                        @error('name')
                    ...
                    <div class="flex flex-col gap-2.5 col-span-2">
                        ...
                        <textarea placeholder="{{ $lang->data['address'] ?? 'Address' }}" wire:model="address"
                            class="p-3 resize-none input min-h-20"></textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div>
                ...
                <div class="scrollable-x-auto">
                    <table class="table table-auto table-border ">
                        <thead class="">
                            <tr>
                                <th class="tw-20">{{ $lang->data['menu_title'] ?? 'Menu Title' }}
                                </th>
                        ...
                        <tbody>
                            @php
                                ...
                                $permission_group = App\Models\Permission::groupBy('list', 'category')
                                    ->select('list', 'category', DB::raw('count(*) as total'))
                                    ->orderBy('list')
                                    ->get();
                            @endphp
                                            ...
                                            @if ($permission->list == $row->list)
                                                <label class="form-check form-check-inline">
                                                    <input class="form-check-input" type="checkbox"
                                                        id="{{ $row->id }}"
                                                        wire:model="selected_permissions.{{ $row->id }}">
                    ...
                    <button type="submit" class="float-end btn btn-primary " wire:click.prevent="save">
                        {{ $lang->data['save'] ?? 'Save' }}
                    </button> 
                </div>  
            </div>     
                    ...
                    <div class="scrollable-x-auto">
                        <div class="grid gap-5 px-0 py-5">
                            <div class="grid grid-cols-2 gap-5 px-4">
                                ...
                                <div class="flex flex-col gap-2.5">
                                    <label class="form-label">
                                        {{ $lang->data['staff_name'] ?? 'Staff Name' }} <span
                                    ...
                                    <div class="row px-2">
                                        <table class="table table-auto table-border ">
                                            ...
                                            <tbody>
                                                @php
                                                    $permission_individual = App\Models\Permission::latest()->get();
                                                    $permission_group = App\Models\Permission::groupBy(
                                                        'list',
                                                        'category',
                                                    )
                                                        ->select('list', 'category', DB::raw('count(*) as total'))
                        ...
                        <button type="submit" class=" mx-3 float-end btn btn-primary " wire:click.prevent="save">
                            {{ $lang->data['save'] ?? 'Save' }}
                        </button>
                    </div>
                </div>
