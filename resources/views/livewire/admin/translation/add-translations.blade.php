                ...
                <div class="flex flex-wrap px-4 items-center justify-between w-full gap-3 container-fluid">
                    <div class="flex flex-wrap items-center gap-1 lg:gap-5">
                        <h1 class="text-lg font-medium text-gray-900">
                            {{ $lang->data['add_translation'] ?? 'Add Translation' }}
                        </h1>
                    </div>
...
<div class="card-body grid gap-5">
    <div class="flex items-center flex-wrap lg:flex-nowrap gap-2.5">
        <label class="form-label max-w-56">
            {{ $lang->data['language_name'] ?? 'Language Name' }}*
        </label>
        <div class="grow">
            <input class="input" placeholder="{{ $lang->data['enter_language_name'] ?? 'Enter language name' }}"
    ...
    <div class="flex items-center flex-wrap lg:flex-nowrap gap-2.5">
        ...
        <div class="grow">
            <input class="input pt-3" placeholder="" type="file" wire:model='icon' />
            @error('icon')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
            ...
            <div class="mb-3 col-md-12">
                ...
                @foreach ($value['values'] as $key => $default)
                    <div class="form-group mb-3 row">
                        <label class="col-3 col-form-label">{{ ucwords(str_replace('_', ' ', $key)) }}</label>
                        <div class="col">
                            <input type="text" class="" wire:model="data.{{ $key }}">
                        </div>
                    </div>
                @endforeach
            </div>
